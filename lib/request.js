  ...
  function makeRequest(req) {
    ...
    return function (callback, results) {
      ...
      var _req = request(opts, function (err, res, body) {
        if (err) {
            // log opts with message (request returned error)
          callback(err, body, res);
          self.emit('error', this, err, body, res);
          return;
        }
        ...
        if (typeof body === 'string') {
          try {
            ...
            // log opts with message (request returned string but not json)
